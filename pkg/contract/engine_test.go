...
func TestExecuteRulesRejectsNil(t *testing.T) {
	if _, err := NewSmartContractEngine().ExecuteRules(nil); err == nil {
...
func TestExecuteRulesRejectsInvalidIdentity(t *testing.T) {
	cases := []struct {
		name   string
		mutate func(*types.PaymentTransaction)
	}{
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			tx := validTransaction()
			tc.mutate(tx)
			if _, err := NewSmartContractEngine().ExecuteRules(tx); err == nil {
...
func TestExecuteRulesCalculatesFee(t *testing.T) {
	...
	if !receipt.Approved || receipt.FeeApplied != 10 || receipt.NetAmount != 9990 {
...
func TestExecuteRulesRejectsFutureTimestamp(t *testing.T) {
	tx := validTransaction()
	tx.Timestamp = time.Now().Add(6 * time.Minute)
	if _, err := NewSmartContractEngine().ExecuteRules(tx); err == nil || !strings.Contains(err.Error(), "future") {
