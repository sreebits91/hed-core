...
func NewPipeline(db plugin.StateEngine, initialShards int) *Pipeline {
	if initialShards < 1 { initialShards = 1 }
	engName := "none"
	if db != nil { engName = db.Name() }
...
func (p *Pipeline) SetStorageEngine(db plugin.StateEngine) {
	p.mu.Lock()
	p.db = db
	if db != nil { p.engineName = db.Name() } else { p.engineName = "none" }
	name := p.engineName
	p.mu.Unlock()
...
func (p *Pipeline) SetShards(count int) {
	if count < 1 { return }
	p.mu.Lock(); p.shards = count; p.mu.Unlock()
...
func (p *Pipeline) SubmitTransaction(tx *TxPayload) (string, int64, error) {
	start := time.Now()
	if tx == nil { atomic.AddUint64(&p.failed, 1); return "", 0, fmt.Errorf("transaction is nil") }
	p.mu.RLock(); shards, db, engineName := p.shards, p.db, p.engineName; p.mu.RUnlock()
	if shards < 1 { return "", 0, fmt.Errorf("invalid shard count: %d", shards) }
	shardIdx := 0
	if len(tx.TxUUID) > 0 { shardIdx = int(tx.TxUUID[0]) % shards }
	...
	if committedCount%1000 == 0 { p.EmitEvent(EventCommit, shardName, tx.TxUUID, fmt.Sprintf("Persisted state [%s] balance=%d via %s", tx.AccountID, tx.Amount, engineName), latUs) }
...
func (p *Pipeline) EngineName() string { p.mu.RLock(); name := p.engineName; p.mu.RUnlock(); return name }

func (p *Pipeline) SubscribeEvents() chan Event { ch := make(chan Event, 500); p.subMux.Lock(); p.subscribers[ch] = true; p.subMux.Unlock(); return ch }
func (p *Pipeline) UnsubscribeEvents(ch chan Event) { p.subMux.Lock(); if _, ok := p.subscribers[ch]; ok { delete(p.subscribers, ch); close(ch) }; p.subMux.Unlock() }
...
func (p *Pipeline) EmitEvent(typ EventType, shard, txUUID, msg string, latUs int64) {
	...
	p.subMux.RLock(); defer p.subMux.RUnlock()
	for ch := range p.subscribers { select { case ch <- evt: default: } }
}

func GenerateUUID() string {
	buf := make([]byte, 16)
	if _, err := rand.Read(buf); err != nil { return fmt.Sprintf("%016x", time.Now().UnixNano()) }
