...
// Run256WorkerBenchmark runs a concurrent in-memory delta benchmark. Key generation
// happens before the timed window so TPS measures the update path only.
func Run256WorkerBenchmark(ctx context.Context, dEngine *delta.DeltaEngine, totalTx int, numWorkers int) Result {
	...
	for w := 0; w < numWorkers; w++ {
		count := totalTx / numWorkers
		if w < totalTx%numWorkers {
			count++
		}
