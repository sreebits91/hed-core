...
func(p *Pipeline) Recover(ctx context.Context)(RecoveryReport,error){
	...
	for _,tx:=range txs{if err:=ctx.Err();err!=nil{return report,err};idx:=tx.Partition;if idx<0||idx>=len(p.parts){return report,ErrInvalidConfig};if tx.Sequence>p.parts[idx].seq.Load(){p.parts[idx].seq.Store(tx.Sequence)};if err:=p.parts[idx].q.Push(tx);err!=nil{return report,err};report.Replayed++;atomic.AddUint64(&p.metrics.partitions[idx].accepted,1);atomic.StoreUint64(&p.metrics.partitions[idx].queueDepth,uint64(p.parts[idx].q.Len()))}
