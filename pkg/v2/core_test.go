...
func TestCommitRetry(t *testing.T){b:=&testBackend{fail:2};c:=NewCommitter(b,RetryPolicy{MaxAttempts:4,InitialBackoff:time.Millisecond,MaxBackoff:2*time.Millisecond},100*time.Millisecond);if err:=c.Commit(context.Background(),Tx{ID:"a"});err!=nil{t.Fatal(err)};if c.RetryCount()!=2{t.Fatalf("retries=%d",c.RetryCount())}}
func TestCommitPermanentFailure(t *testing.T){b:=&testBackend{fail:10};c:=NewCommitter(b,RetryPolicy{MaxAttempts:4,InitialBackoff:time.Millisecond,MaxBackoff:2*time.Millisecond},100*time.Millisecond);err:=c.Commit(context.Background(),Tx{ID:"a"});if err==nil{t.Fatal("expected failure")};if c.RetryCount()!=3{t.Fatalf("retries=%d",c.RetryCount())}}
