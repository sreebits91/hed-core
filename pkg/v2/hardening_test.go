...
func TestWALAbortDoesNotReplay(t *testing.T) {
	...
	if err := w.Abort(tx.ID); err != nil { t.Fatal(err) }
	got, err := w.Replay()
	if err != nil { t.Fatal(err) }
	if len(got) != 0 { t.Fatalf("replayed aborted transaction: %#v", got) }
...
func TestRecoveryRestoresSequenceCounter(t *testing.T) {
	...
	if err := w.Append(tx); err != nil { t.Fatal(err) }
	if err := w.Close(); err != nil { t.Fatal(err) }
	cfg := DefaultConfig(); cfg.Partitions = 1; cfg.QueueCapacity = 128; cfg.WALPath = path
	...
	if report.Replayed != 1 { t.Fatalf("replayed=%d want=1", report.Replayed) }
	if got := p.parts[0].seq.Load(); got != 41 { t.Fatalf("sequence=%d want=41", got) }
	second, err := p.Recover(context.Background()); if err != nil { t.Fatal(err) }
...
func TestConcurrentIngressIsLossless(t *testing.T) {
	cfg := DefaultConfig(); cfg.Partitions = 8; cfg.QueueCapacity = 16384; cfg.BatchSize = 256; cfg.WALPath = ""
	p, err := NewPipeline(cfg, nil); if err != nil { t.Fatal(err) }
	const producers = 16; const perProducer = 500
	var wg sync.WaitGroup; var mu sync.Mutex; accepted := 0
	for producer := 0; producer < producers; producer++ {
		producer := producer; wg.Add(1)
		go func() {
			defer wg.Done()
			for i := 0; i < perProducer; i++ {
				_, err := p.Submit(context.Background(), Tx{ID: "concurrent-"+strconv.Itoa(producer)+"-"+strconv.Itoa(i), Key: "account-"+strconv.Itoa(i%64), Payload: []byte("payload")})
				if err != nil { t.Errorf("unexpected submit error: %v", err); return }
...
type fakeLedger struct { status LedgerTxStatus; err error; calls int }
func (f *fakeLedger) Status(context.Context, string) (LedgerTxStatus, error) { f.calls++; return f.status, f.err }

func TestReconcilerClassifiesLedgerState(t *testing.T) {
	f := &fakeLedger{status: LedgerCommitted}; r := NewReconciler(f)
	...
	if _, err = r.Reconcile(context.Background(), "tx-3"); err != ErrLedgerStateUnknown { t.Fatalf("expected unknown-state error, got %v", err) }
}
...
func TestQueueFullAbortsWALAndAllowsRetry(t *testing.T) {
	path := filepath.Join(t.TempDir(), "hed.wal")
	cfg := DefaultConfig()
	cfg.Partitions = 1; cfg.QueueCapacity = 1; cfg.WALPath = path; cfg.BatchSize = 1; cfg.FlushInterval = time.Hour
	b := &blockingBackend{started: make(chan struct{}), allow: make(chan struct{}), committed: make(chan string, 3)}
	p, err := NewPipeline(cfg, b); if err != nil { t.Fatal(err) }
	...
	close(b.allow)
	waitForCommit(t, b.committed, tx1.ID)
	...
	txs, err := w.Replay(); if err != nil { t.Fatal(err) }
	for _, tx := range txs {
		if tx.ID == tx3.ID { t.Fatalf("aborted transaction remained pending after retry: %+v", tx) }
	}
}
