...
func TestWALAbortDoesNotReplay(t *testing.T) {
    ...
    got, err := w.Replay()
    if err != nil { t.Fatal(err) }
    if len(got) != 0 { t.Fatalf("replayed aborted transactions: %#v", got) }
...
func TestRecoveryRestoresSequenceCounter(t *testing.T) {
    ...
    if _, err := p.Submit(context.Background(), Tx{ID: "after-recovery", Key: "k", Payload: []byte("x")}); err != nil {
        t.Fatal(err)
    }
    if got := p.parts[0].seq.Load(); got != 42 { t.Fatalf("sequence=%d want=42", got) }
}
