...
func(q *Queue)PopBatchInto(out []Tx,n int)([]Tx,bool){if n<=0{n=1};var first Tx;select{case first=<-q.items:q.depth.Add(-1);default:return nil,q.closed.Load()};if cap(out)<n{out=make([]Tx,n)}else{out=out[:n]};out[0]=first;count:=1;for count<n{select{case tx:=<-q.items:out[count]=tx;count++;q.depth.Add(-1);default:return out[:count],false}};return out[:count],false}
...
func(d *Dedup)compactLocked(s *dedupShard){if len(s.order)==0{return};out:=s.order[:0];for _,id:=range s.order{if _,ok:=s.m[id];ok{out=append(out,id)}};s.order=out}
func(d *Dedup)expireLocked(s *dedupShard,now time.Time){i:=0;for i<len(s.order){id:=s.order[i];e,ok:=s.m[id];if !ok||!now.Before(e.expires){delete(s.m,id);i++;continue};break};if i>0{copy(s.order,s.order[i:]);s.order=s.order[:len(s.order)-i]}}
...
func(w *WAL)Replay()([]Tx,error){if w==nil{return nil,nil};w.mu.Lock();defer w.mu.Unlock();if _,e:=w.f.Seek(0,0);e!=nil{return nil,e};r:=bufio.NewReader(w.f);pending:=map[string]Tx{};for{line,e:=r.ReadBytes('\n');if e!=nil&&len(line)==0{if e==io.EOF{break};return nil,e};line=[]byte(strings.TrimSpace(string(line)));if len(line)==0{if e!=nil{break};continue};var rec WALRecord;if x:=json.Unmarshal(line,&rec);x!=nil{if e==io.EOF{break};return nil,ErrWALCorrupt};if rec.Checksum!=recordChecksum(rec.Kind,rec.Tx,rec.ID){return nil,ErrWALCorrupt};switch rec.Kind{case"prepare":pending[rec.Tx.ID]=rec.Tx;case"commit","abort":delete(pending,rec.ID);default:return nil,ErrWALCorrupt};if e!=nil{break}};out:=make([]Tx,0,len(pending));for _,tx:=range pending{out=append(out,tx)};sort.Slice(out,func(i,j int)bool{if out[i].Partition==out[j].Partition{return out[i].Sequence<out[j].Sequence};return out[i].Partition<out[j].Partition});return out,nil}
...
type Pipeline struct{cfg Config;router *Router;dedup *Dedup;wal *WAL;committer *Committer;metrics *Metrics;parts []partition;ctx context.Context;cancel context.CancelFunc;accepting atomic.Bool;recoveryMu sync.Mutex;recovered map[string]struct{};wg sync.WaitGroup;startOnce,stopOnce sync.Once}
func NewPipeline(cfg Config,backend CommitBackend)(*Pipeline,error){if e:=cfg.Validate();e!=nil{return nil,e};r,_:=NewRouter(cfg.Partitions);d,_:=NewDedup(cfg.DedupCapacity,cfg.DedupTTL);w,e:=OpenWAL(cfg.WALPath,cfg.SyncWAL);if e!=nil{return nil,e};ctx,cancel:=context.WithCancel(context.Background());p:=&Pipeline{cfg:cfg,router:r,dedup:d,wal:w,committer:NewCommitter(backend,cfg.Retry,cfg.CommitTimeout),metrics:newMetrics(cfg.Partitions),parts:make([]partition,cfg.Partitions),ctx:ctx,cancel:cancel,recovered:make(map[string]struct{},cfg.DedupCapacity)};p.accepting.Store(true);for i:=range p.parts{p.parts[i].q,_=NewQueue(cfg.QueueCapacity)};p.start();return p,nil}
func(p *Pipeline)start(){p.startOnce.Do(func(){for i:=range p.parts{p.wg.Add(1);go p.worker(i)}})}
func(p *Pipeline)worker(idx int){defer p.wg.Done();ticker:=time.NewTicker(p.cfg.FlushInterval);defer ticker.Stop();batchBuf:=make([]Tx,p.cfg.BatchSize);var expected uint64;var initialized bool;for{batch,closed:=p.parts[idx].q.PopBatchInto(batchBuf,p.cfg.BatchSize);if len(batch)>0{for _,tx:=range batch{if !initialized{expected=tx.Sequence-1;initialized=true};if tx.Sequence!=expected+1{atomic.AddUint64(&p.metrics.partitions[idx].orderingErrors,1)};expected=tx.Sequence};p.flush(idx,batch)};if closed{return};select{case<-ticker.C:case<-p.parts[idx].q.notify:case<-p.ctx.Done():for{b,c:=p.parts[idx].q.PopBatchInto(batchBuf,p.cfg.BatchSize);if len(b)>0{for _,tx:=range b{if !initialized{expected=tx.Sequence-1;initialized=true};if tx.Sequence!=expected+1{atomic.AddUint64(&p.metrics.partitions[idx].orderingErrors,1)};expected=tx.Sequence};p.flush(idx,b);continue};if c{return};break}};atomic.StoreUint64(&p.metrics.partitions[idx].queueDepth,uint64(p.parts[idx].q.Len()))}}
...
func(p *Pipeline)Stop(){p.stopOnce.Do(func(){p.accepting.Store(false);for i:=range p.parts{p.parts[i].q.Close()};p.cancel();p.wg.Wait();if p.wal!=nil{_=p.wal.Close()}})}
