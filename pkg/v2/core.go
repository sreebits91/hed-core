...
func(p *Pipeline)worker(idx int){defer p.wg.Done();ticker:=time.NewTicker(p.cfg.FlushInterval);defer ticker.Stop();var expected uint64;var initialized bool;for{batch,closed:=p.parts[idx].q.PopBatch(p.cfg.BatchSize);if len(batch)>0{for _,tx:=range batch{if !initialized{expected=tx.Sequence-1;initialized=true};if tx.Sequence!=expected+1{atomic.AddUint64(&p.metrics.partitions[idx].orderingErrors,1)};expected=tx.Sequence};p.flush(idx,batch)};if closed{return};select{case<-ticker.C:case<-p.parts[idx].q.notify:case<-p.ctx.Done():for{b,c:=p.parts[idx].q.PopBatch(p.cfg.BatchSize);if len(b)>0{for _,tx:=range b{if !initialized{expected=tx.Sequence-1;initialized=true};if tx.Sequence!=expected+1{atomic.AddUint64(&p.metrics.partitions[idx].orderingErrors,1)};expected=tx.Sequence};p.flush(idx,b);continue};if c{return};break}};atomic.StoreUint64(&p.metrics.partitions[idx].queueDepth,uint64(p.parts[idx].q.Len()))}}
