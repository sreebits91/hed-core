...
func(d *Dedup)Forget(id string){d.mu.Lock();defer d.mu.Unlock();if _,ok:=d.m[id];!ok{return};delete(d.m,id);if len(d.order)>d.capacity*2{d.compactLocked()}}
func(d *Dedup)compactLocked(){if len(d.order)==0{return};out:=d.order[:0];for _,id:=range d.order{if _,ok:=d.m[id];ok{out=append(out,id)}};d.order=out}
func(d *Dedup)expireLocked(now time.Time){i:=0;for i<len(d.order){id:=d.order[i];e,ok:=d.m[id];if !ok||!now.Before(e.expires){delete(d.m,id);i++;continue};break};if i>0{copy(d.order,d.order[i:]);d.order=d.order[:len(d.order)-i]}}
