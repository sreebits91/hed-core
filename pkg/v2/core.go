...
func(p *Pipeline)worker(idx int){defer p.wg.Done();ticker:=time.NewTicker(p.cfg.FlushInterval);defer ticker.Stop();var expected uint64;var initialized bool;for{batch,closed:=p.parts[idx].q.PopBatch(p.cfg.BatchSize);if len(batch)>0{for _,tx:=range batch{if !initialized{expected=tx.Sequence-1;initialized=true};if !initialized{expected=tx.Sequence-1;initialized=true};if tx.Sequence!=expected+1{atomic.AddUint64(&p.metrics.partitions[idx].orderingErrors,1)};expected=tx.Sequence};p.flush(idx,batch)};if closed{return};select{case<-ticker.C:case<-p.ctx.Done():for{b,c:=p.parts[idx].q.PopBatch(p.cfg.BatchSize);if len(b)>0{for _,tx:=range b{if !initialized{expected=tx.Sequence-1;initialized=true};if tx.Sequence!=expected+1{atomic.AddUint64(&p.metrics.partitions[idx].orderingErrors,1)};expected=tx.Sequence};p.flush(idx,b);continue};if c{return};break}};atomic.StoreUint64(&p.metrics.partitions[idx].queueDepth,uint64(p.parts[idx].q.Len()))}}
...
func(p *Pipeline)ReplayUncommitted()([]Tx,error){if p.wal==nil{return nil,nil};txs,e:=p.wal.Replay();if e!=nil{return nil,e};p.metrics.recovered.Add(uint64(len(txs)));return txs,nil}\ntype RecoveryReport struct{Replayed int;Rejected int}\nfunc(p *Pipeline)Recover(ctx context.Context)(RecoveryReport,error){var report RecoveryReport;txs,e:=p.ReplayUncommitted();if e!=nil{return report,e};for _,tx:=range txs{if err:=ctx.Err();err!=nil{return report,err};if tx.Partition<0||tx.Partition>=len(p.parts){report.Rejected++;continue};if tx.Sequence>p.parts[tx.Partition].seq.Load(){p.parts[tx.Partition].seq.Store(tx.Sequence)};if p.dedup.SeenOrAdd(tx.ID,time.Now()){continue};if e:=p.parts[tx.Partition].q.Push(tx);e!=nil{report.Rejected++;p.dedup.Forget(tx.ID);continue};atomic.AddUint64(&p.metrics.partitions[tx.Partition].accepted,1);report.Replayed++};return report,nil}
