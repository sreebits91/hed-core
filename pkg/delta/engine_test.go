...
func (m *mockState) BatchWriteWithID(requestID, channel string, updates map[string]int64) error {
    ...
    if m.fail { return errors.New("temporary failure") }
...
func TestFlushRetriesFailedBatchWithSameRequestID(t *testing.T) {
    ...
    if err := d.FlushToDB("channel1"); err == nil { t.Fatal("expected persistence failure") }

    store.fail = false
    if err := d.FlushToDB("channel1"); err != nil { t.Fatal(err) }
    if len(store.writes) != 1 { t.Fatalf("expected one successful logical write, got %d", len(store.writes)) }
}
...
func TestChannelIsolation(t *testing.T) {
    ...
    if err := d.FlushToDB("a"); err != nil { t.Fatal(err) }
    if len(store.writes) != 1 { t.Fatalf("expected only channel a to flush, got %d", len(store.writes)) }
    if err := d.FlushToDB("b"); err != nil { t.Fatal(err) }
    if len(store.writes) != 2 { t.Fatalf("expected channel b to remain pending, got %d writes", len(store.writes)) }
}
