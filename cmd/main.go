...
func main() {
	...
	if err := keydbEngine.Init(nil); err != nil { log.Fatalf("❌ [FATAL] KeyDB Connection Failed: %v", err) }
	...
	go func() { ticker := time.NewTicker(10 * time.Millisecond); defer ticker.Stop(); for { select { case <-ctx.Done(): return; case <-ticker.C: _ = deltaEngine.FlushToDB("channel1") } } }()
	log.Println("✅ [5/5] Background Async DB Pipeline Active")
	fmt.Println("\n-----------------------------------------------------------------")
	log.Println("📥 [STEP 1: PAYLOAD INGESTION] Receiving payment payload...")
	sampleTx := &types.PaymentTransaction{TxID:"TX_88492049102", SenderID:"acc_alice", ReceiverID:"acc_bob", Amount:50000, Currency:"USD", ChannelID:"channel1", Timestamp:time.Now(), Signature:"sig_secp256k1_3a8f9c1b..."}
	...
	if err != nil { log.Fatalf("❌ Transaction rejected by contract: %v", err) }
